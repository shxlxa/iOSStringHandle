...
# 将excel表sheet2中的中文数据与sheet1中的中文进行对比，中文相同时
# 将sheet1中的key替换成sheet2dekey,写入到一个新的excel表中
...
def shizhan2():
	...
	sheet1 = data.sheets()[0] # 打开第一张表

	sheet2 = data.sheets()[1] # 打开第二张表
	nrows = sheet2.nrows      # 获取表的行数
	ncols = sheet2.ncols		 # 获取表的列数
	...
	for row in range(sheet1.nrows):
		...
		print(cell)
		# 依次读取sheet2第一列的数据	
		for rowB in range(sheet2.nrows):
			cellB = sheet2.cell_value(rowB,2)
			if cell == cellB:
				print 'row '+str(rowB)+' '+cell
				# print 'row ' + str(row) + ' ' + sheet2.cell_value(rowB,0)
				# 1.将android_key写入到第一列中
				android_key = sheet2.cell_value(rowB,0)
